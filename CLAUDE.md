...
- `CLAUDE.md` (this file, inside the `<laravel-boost-guidelines>` tags below) and `AGENTS.md` are generated by `php artisan boost:install` / `boost:update`, and `boost:update` runs on every `composer update` via `post-update-cmd`. **Never edit inside those tags** — write custom guidance above them, as this section is.
- Boost skills (`laravel-best-practices`, `pest-testing`, `tailwindcss-development`) are mirrored into both `.claude/skills/` and `.agents/skills/`; configured in `boost.json`.
- The `laravel-boost` MCP server is wired up in `.mcp.json` and enabled in `.claude/settings.local.json`.
